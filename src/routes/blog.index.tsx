    ...
    <main className="page-wrap px-4 pb-8 pt-8">
      <section className="mb-4">
        ...
        <p className="mt-3 max-w-3xl text-sm text-[var(--sea-ink-soft)] sm:text-base">
          Field notes on x402 payments, MCP infrastructure, and building cloud
      ...
      {featured ? (
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <article className="accent-shell rise-in rounded-2xl p-5 sm:p-6 lg:col-span-2">
            {featured.heroImage ? (
              <img
          ...
          {posts.map((post, index) => (
            <article
              key={post.slug}
              className="band-shell rise-in rounded-2xl p-5 sm:last:col-span-2 lg:last:col-span-1"
        ...
        <section className="band-shell rounded-2xl p-6 text-sm text-[var(--sea-ink-soft)]">
          First EquiForge dispatch is in progress. Check back soon.
        </section>
      )}
    </main>
  )
